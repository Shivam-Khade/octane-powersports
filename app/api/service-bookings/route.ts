...
export async function POST(req: Request) {
  ...
  try {
    const { name, phone, email, bikeModel, date, serviceType, notes } = await req.json();

    if (!name || !phone || !email || !bikeModel || !date || !serviceType) {
      return NextResponse.json({ error: "All required fields must be provided" }, { status: 400 });
    }

    const [result] = await pool.query(
      "INSERT INTO service_bookings (user_id, name, phone, email, bike_model, service_type, booking_date, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
      [session.user.id, name, phone, email, bikeModel, serviceType, date, notes || null]
    );
